...
import {
    ...
    buildSignatureBytes,
    executeContractGlobalCallWithSigners,
...
describe("Safe", async () => {
    ...
    describe("execGlobalTransaction", async () => {
        it("should revert if too little gas is provided", async () => {
            ...
            const tx = buildSafeTransaction({ to: safe.address, safeTxGas: 1000000, nonce: await safe.globalNonce() });
            const signatureBytes = buildSignatureBytes([await safeGlobalApproveHash(user1, safe, tx, true)]);
            await expect(
                safe.execGlobalTransaction(
                    tx.to,
                    tx.value,
                    tx.data,
                    tx.operation,
        ...
        it("should emit event for successful call execution", async () => {
            ...
            const txHash = calculateSafeTransactionHash(
                safe,
                buildContractCall(storageSetter, "setStorage", ["0xbaddad"], await safe.globalNonce()),
                0,
            );
            await expect(executeContractGlobalCallWithSigners(safe, storageSetter, "setStorage", ["0xbaddad"], [user1]))
        ...
        it("should emit event for failed call execution if safeTxGas > 0", async () => {
            ...
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1], false, { safeTxGas: 1 })).to.emit(
                safe,
                "ExecutionFailure",
            );
        });
        ...
        it("should emit event for failed call execution if gasPrice > 0", async () => {
            ...
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1], false, { gasPrice: 1 })).to.emit(
                safe,
                "ExecutionFailure",
            );
        });
        ...
        it("should revert for failed call execution if gasPrice == 0 and safeTxGas == 0", async () => {
            const { safe, reverter } = await setupTests();
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1])).to.revertedWith("GS013");
        });

        it("should emit event for successful delegatecall execution", async () => {
            const { safe, storageSetter } = await setupTests();
            await expect(executeContractGlobalCallWithSigners(safe, storageSetter, "setStorage", ["0xbaddad"], [user1], true)).to.emit(
                safe,
                "ExecutionSuccess",
            );
        ...
        it("should emit event for failed delegatecall execution  if safeTxGas > 0", async () => {
            ...
            const txHash = calculateSafeTransactionHash(
                safe,
                buildContractCall(reverter, "revert", [], await safe.globalNonce(), true, { safeTxGas: 1 }),
                0,
            );
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1], true, { safeTxGas: 1 }))
        ...
        it("should emit event for failed delegatecall execution if gasPrice > 0", async () => {
            const { safe, reverter } = await setupTests();
            await user1.sendTransaction({ to: safe.address, value: 10000000 });
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1], true, { gasPrice: 1 })).to.emit(
                safe,
                "ExecutionFailure",
            );
        });
        ...
        it("should emit event for failed delegatecall execution if gasPrice == 0 and safeTxGas == 0", async () => {
            const { safe, reverter } = await setupTests();
            await expect(executeContractGlobalCallWithSigners(safe, reverter, "revert", [], [user1], true)).to.revertedWith("GS013");
        });

        it("should revert on unknown operation", async () => {
            const { safe } = await setupTests();
            const tx = buildSafeTransaction({ to: safe.address, nonce: await safe.globalNonce(), operation: 2 });
            await expect(executeGlobalTx(safe, tx, [await safeGlobalApproveHash(user1, safe, tx, true)])).to.be.reverted;
        });
        ...
        it("should emit payment in success event", async () => {
            ...
            let executedTx: any;
            await expect(
                executeGlobalTx(safe, tx, [await safeGlobalApproveHash(user1, safe, tx, true)]).then((tx) => {
                    executedTx = tx;
                    return tx;
                }),
            ).to.emit(safe, "ExecutionSuccess");
            ...
            expect(successEvent.txHash).to.be.eq(calculateSafeTransactionHash(safe, tx, 0));
        ...
        it("should be possible to manually increase gas", async () => {
            ...
            const tx = buildSafeTransaction({ to, data, safeTxGas, nonce: await safe.globalNonce() });
            await expect(
                executeGlobalTx(safe, tx, [await safeGlobalApproveHash(user1, safe, tx, true)], { gasLimit: 170000 }),
                "Safe transaction should fail with low gasLimit",
            ).to.emit(safe, "ExecutionFailure");

            await expect(
                executeGlobalTx(safe, tx, [await safeGlobalApproveHash(user1, safe, tx, true)], { gasLimit: 6000000 }),
